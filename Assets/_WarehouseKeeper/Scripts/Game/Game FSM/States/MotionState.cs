...
internal class MotionState : State<Vector2, bool>
{
    ...
    private async void MovePerson(Vector2 direction)
    {
        ...
        if (_levelDirector.ActiveLevel.Hint.InProgress &&
            _levelDirector.ActiveLevel.Hint.GetActiveDirection() != direction)
        {
            ExitedState = true;
        ...
        {
            var currentBoxPos =
                new Vector2Int(level.GetPlayer().X + intDirection.x, level.GetPlayer().Y + intDirection.y);
                                    ...
                                    GameEntityType.Box))
            {
                                        ...
                                        GameEntityType.None))
                {
                    ...
                    //View moving
                    var entityTask = MoveEntityView(level.nodes, level.entities[currentBoxPos.x, currentBoxPos.y],
                                                    intDirection, level.entities);
    ...
    private bool StayOnWayGameEntity(ref GameEntity[,] nodes, int activeX, int activeY, Vector2Int direction,
                                     params GameEntityType[] types)
    {
    ...
    private async Task MoveEntityView(Node[,] nodes, GameEntity gameEntity, Vector2Int direction,
                                      GameEntity[,] entities)
    {
